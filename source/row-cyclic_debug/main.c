...
int main (int argc, char **argv) {
	...
	//Partial Pivoting
	get_pp_parameters (x, y, &pp_max, &pp_loc);
	local_in.value = pp_max;
	local_in.rank = rank;
	MPI_Allreduce (&local_in, &global_out, 1, MPI_DOUBLE_INT, MPI_MAXLOC, owner_rank, MPI_COMM_WORLD);
	...
	if (rank == owner_rank) {
		...
		dest = j;
		MPI_Send (&matrix[index(x,0)], 1, MPI_ROW, dest, tag1, MPI_COMM_WORLD);
	    }
